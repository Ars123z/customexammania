...
from .models import Exam, Book, Chapter, Exercise, ExamQuestion, UserSubmittedBookAnswer, BookQuestionFeedback,BookQuestion
...
def generate_test(request, subject):
    try:
        user_profile = UserProfile.objects.get(user=request.user)
        if user_profile.is_subscriber and user_profile.subscription_end_date > datetime.now(user_profile.subscription_end_date.tzinfo):
            if subject not in ['physics', 'chemistry', 'math']:
                return HttpResponse("Invalid subject")

            book_questions = BookQuestion.objects.filter(subject=subject).order_by('?')[:5]
            exam_questions = ExamQuestion.objects.filter(subject=subject).order_by('?')[:5]

            book_questions_data = [{'body': question.body, 'answer': question.answer, 'options': list(question.options.values_list('content', flat=True)), 'correct_options': list(question.correct_options.values_list('content', flat=True))} for question in book_questions]
            exam_questions_data = [{'body': question.body, 'answer': question.answer, 'options': list(question.options.values_list('content', flat=True)), 'correct_options': list(question.correct_options.values_list('content', flat=True))} for question in exam_questions]

            all_questions_data = book_questions_data + exam_questions_data
            all_questions_data = sample(all_questions_data, len(all_questions_data))

            return JsonResponse({'questions': all_questions_data})
...
@login_required
def test_page(request, subject):
    questions = generate_test(request, subject)
    if isinstance(questions, JsonResponse):
        questions_dict = json.loads(questions.content)
        ...
        return render(request, 'main/test.html', dict)
    else:
       return HttpResponse("<h1>We incountered an error</h1>")
